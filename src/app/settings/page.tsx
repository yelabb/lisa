...
export default function SettingsPage() {
  ...
  // Sync with store when page loads
  useEffect(() => {
    queueMicrotask(() => {
  ...
  }, [language, progress?.preferredThemes, progress?.difficultyMultiplier]);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-lg z-50 pb-safe">
        <div className="max-w-lg mx-auto flex gap-3 p-4">
          <button
            onClick={handleBack}
            className="flex-1 px-4 py-3 rounded-xl border border-gray-300 text-gray-600 font-medium hover:bg-gray-100 transition-colors"
          >
            {tCommon('cancel')}
          </button>
          <button
